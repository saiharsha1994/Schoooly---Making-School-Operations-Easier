<?php
/* only text message */
$messages = $this->db->get_where('message', array('message_thread_code' => $current_message_thread_code, 'post_type' => '1'))->result_array();
        ...
        <div class="mail-sender " style="padding:7px;">
            ...
            <a href="#" class="dropdown-toggle" data-toggle="dropdown">
				...
				if($sender_account_type!='parent'){
						echo $this->db->get_where('employee_details', array('emp_id' => $sender_id))->row()->name; 	
					}else{
						echo $this->db->get_where('parent', array('parent_id' => $sender_id))->row()->name; 
					}
...
<?php 
				if($row['read_status']==0)
					echo "<p style=color:red;>UnRead</p>";
				else
					echo "<p style=color:blue;>Read</p>";?> 			
        </div>
...
<?php echo form_open(base_url() . 'index.php?admin/private_message/send_reply/' . $current_message_thread_code, array('enctype' => 'multipart/form-data')); ?>
