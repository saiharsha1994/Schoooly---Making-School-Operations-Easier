...
class Login extends CI_Controller {
    ...
    function ajax_login() {
        ...
        $email = $_POST["email"];
        $password = $this->apicrypter->encrypt($_POST["password"]);
    ...
    function validate_login($email = '', $password = '') {
		...
		$AdminCredential = array('login' => $email, 'password' => $password);
        $ParentCredential = array('email' => $email, 'password' => $password);
		$query = $this->db->get_where('employee_details', $AdminCredential);
        if ($query->num_rows() > 0) {
            $row = $query->row();
			$teacher_role_id=$this->db->get_where('hr_roles' , array('role' => 'teacher'))->row()->id;
			$admin_role_id=$this->db->get_where('hr_roles' , array('role' => 'admin'))->row()->id;
			...
			//set both login 1, coz we integrating both teacher and Admin in same navigation.
			$this->session->set_userdata('admin_login', '1');
			//$this->session->set_userdata('teacher_login', '1');
			$this->session->set_userdata('teacher_id', $row->emp_id);
			$this->session->set_userdata('admin_id', $row->emp_id);
			$this->session->set_userdata('login_type', 'admin');
			
			$this->session->set_userdata('login_user_id', $row->emp_id);
            $this->session->set_userdata('name', $row->name);
            $this->session->set_userdata('role_id', $row->emp_type);
...
*/
        ...
        $query = $this->db->get_where('parent', $ParentCredential);
        if ($query->num_rows() > 0) {
            $row = $query->row();
    ...
    function ajax_forgot_password()
    {
		...
		// Checking credential for employees
		$query = $this->db->get_where('employee_details', array('login' => $email));
